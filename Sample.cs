...
namespace Example
{
    class Program
    {
        ...
        {
            string filePath = @"C:\Users\harsh\Pictures\tf78500733_win32.dotx";
            string fileResult = @"Result.pdf";
            DocumentCore dc = DocumentCore.Load(filePath);
            foreach (Paragraph par in dc.GetChildElements(true, ElementType.Paragraph))
                foreach (ContentRange item in par.Content.Find("Name of Recipient").Reverse())
                {
                    item.Replace("Harshavardhan K");
                }
            dc.Save(fileResult);
